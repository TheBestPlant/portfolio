document.addEventListener("DOMContentLoaded", () => {
    ...
    if (codeDisplay) {
        ...
        function setActiveButton(activeButton, buttonGroupSelector) {
        ...
        if (fileButtonsContainer) {
            ...
            document.querySelectorAll("[data-folder]").forEach(folderButton => {
                folderButton.addEventListener("click", () => {
                    setActiveButton(folderButton, "[data-folder]");

                    const folderKey = folderButton.dataset.folder;
                    const files = fileMap[folderKey] || [];
                    ...
                    files.forEach((file, index) => {
                        ...
                        btn.addEventListener("click", () => {
                            setActiveButton(btn, "#file-buttons .top-button");
    ...
    // =========================
    const blueprintButtons = document.querySelectorAll(".blueprint-button");
    const blueprints = document.querySelectorAll(".blueprint-item");

    if (blueprintButtons.length > 0) {
        blueprintButtons.forEach(button => {
            button.addEventListener("click", () => {
                const target = button.dataset.blueprint;

                blueprintButtons.forEach(btn => btn.classList.remove("active"));
                blueprints.forEach(section => section.classList.remove("active"));
...
if (modal && modalImg && images.length > 0 && closeBtn) {
    images.forEach(img => {
